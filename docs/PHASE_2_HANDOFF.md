...
- **POPIA cross-border residency — Neon now, AWS Cape Town later.** Phase 2 starts on Neon (`eu-central-1` recommended). Long-term plan: **self-host Postgres in AWS Cape Town (`af-south-1`) on Docker**, so PII never leaves SA jurisdiction. Until then, document the cross-border note in the compliance log *before* the first real user signs up. **Architecture is portable today:** Drizzle is driver-agnostic; the only swap is `drizzle-orm/neon-http` → `drizzle-orm/node-postgres` (or `postgres-js`) in `db/client.ts`, plus the Docker compose + backups + monitoring stack. Schema, queries, seed script, and every page above the data layer stay identical. Tracked as a Phase 9 milestone — see `ROADMAP.md`.
