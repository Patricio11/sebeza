...
- **Phase 9.7** (side-phase between Phase 9 and Phase 10  *nationality analytics & local-hiring intelligence*: 2-class `nationality_class` split + Skills-Shortage Justification Index + Local-Hiring Opportunity Map + employer self-view + `gov`-only per-employer ESA §8 evidence-aid lookup **dormant behind `feature_flag_employer_mix_lookup`** + watch-the-watchers oversight log + printable policy brief. Five structural defences (2-class split, k=10 + complementary suppression, employer floor, dormant-by-default flag, oversight log). Six compliance assertions on `/api/admin/outcomes-compliance` including a structural ban on country-level analytics. Anti-xenophobia tool by construction  framed on EEA §1 + ESA §8 (counsel review of the wording tracked as DPIA R9; engineering work complete, copy is the only blocker on the per-employer + self-view surfaces).)  ✅ done 2026-05-24. See `docs/completed/PHASE_9_7_COMPLETE.md` + `docs/completed/PHASE_9_7_PLAN.md`. Seven new query/library files, four new pages, three new API routes, three migrations (`0012`–`0014`), four new nav entries across gov + admin, 22/22 vitest fixtures green, `npm test` + `npm run test:watch` scripts added (vitest devDep is the runner).
- **Next:** Phase 10  accessibility audit (WCAG 2.2 AA), performance budget on throttled 3G, full Tier-1 + Tier-2 + Tier-3 localisation rollout. Public-launch phase. Outstanding-but-not-blocking from 9.7: DPIA R9 counsel review of the EEA §1 / ESA §8 wording on `<EmployerHiringMixCard>` (DRAFT banner) + `/gov/employer-lookup` + `/gov/opportunity` + `/gov/brief`; `feature_flag_employer_mix_lookup` flip when DEL §8 partnership lands; cron + email distribution of `/gov/brief` as the recurring artefact.
