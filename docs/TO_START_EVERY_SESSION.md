...
- **Next:** Phase 9.7 build continues at Task 9.7.5 (employer self-view: "Your hiring on Sebenza"  scoped to `organizationId = session.org` only, EEA §1 + ESA §8 framing copy held until counsel sign-off on DPIA R9). Then 9.7.6 (per-employer gov lookup, ships dormant), 9.7.7 (oversight log), 9.7.8 (scheduled brief), 9.7.9 (wiring). Then Phase 10  accessibility audit (WCAG 2.2 AA), performance budget on throttled 3G, full Tier-1 + Tier-2 + Tier-3 localisation rollout. Public-launch phase.
