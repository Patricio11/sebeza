...
- **Next:** Phase 8 — Resend transactional emails (flips on the email column on the prefs panel), cron jobs (30-day hard-delete · `status.stale.warning` · `saved_search.new_matches` · nightly skill-gap snapshot), Home Affairs KYC adapter, SAQA qualification adapter. Opening in `docs/PHASE_8_PLAN.md`.
