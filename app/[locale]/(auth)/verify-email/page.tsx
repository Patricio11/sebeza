...
import { ResendVerificationButton } from "@/components/feature/auth/ResendVerificationButton";
...
export default async function VerifyEmailPage({
  ...
  const displayEmail = email || "you@example.co.za";

  return (
    <AuthShell
      eyebrow="Verification"
      heading={t("heading")}
      subhead={t("subhead", { email: displayEmail })}
    >
      <div className="flex flex-col items-start gap-6">
        ...
        <div className="flex flex-wrap gap-3">
          {email ? (
            <ResendVerificationButton email={email} label={t("resend")} />
          ) : (
            <p className="text-sm italic text-[color:var(--color-ink-soft)]">
              Open the verification link from your inbox to finish creating
              your account.
            </p>
          )}
          <Link
