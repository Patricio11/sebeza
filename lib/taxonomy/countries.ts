/**
 * Phase 9.16  ISO 3166-1 alpha-2 country catalogue.
 *
 * Static constant (no DB table) because country codes are universal +
 * ISO-stable. Used by:
 *   - the passport-country picker on sign-up + profile editor
 *   - the assertion that validates `profiles.passport_country` is a
 *     known alpha-2 code
 *   - any future country picker (rare; nationality picker today is a
 *     free-text TextField, deferred per the user's input)
 *
 * Sort: South Africa pinned first since 99% of Sebenza users are SA;
 * then the SADC neighbours; then alphabetical for the rest. Picker
 * UX wins  most users find their country in the first dozen entries.
 */
...
// ── SA + SADC (pinned first for picker UX) ────────────────────────────
const SA_SADC: CountryEntry[] = [
...
// ── Rest of the world (alphabetical by label) ─────────────────────────
...
/** The full picker list  SA + SADC first, then the rest of the world. */
export const COUNTRIES: CountryEntry[] = [...SA_SADC, ...REST_OF_WORLD];

const CODE_TO_LABEL = new Map(COUNTRIES.map((c) => [c.code, c.label]));

/** Quick membership check for the validation helpers + compliance assertion. */
