...
interface Props {
  ...
  /**
   * Phase 31 (data minimisation)  when false (the launch default), the
   * upload affordances are hidden and the server refuses uploads; the
   * verified state + revoke stay fully functional (status display and
   * erasure never switch off). The parent only renders this panel at
   * all when collection is on OR KYC data already exists.
   */
...
export function KycPanel({
  ...
  if (idDocumentRejectionReason) {
    return (
      <div className="rounded-[var(--radius-md)] border-2 border-[color:var(--color-danger)] bg-[color:var(--color-danger)]/10 p-5">
        ...
        {/* Phase 31  re-upload only while collection is enabled. */}
        {collectionEnabled && (
          <>
            <input
              ref={fileRef}
  ...
  if (hasIdDocument) {
    return (
      <div className="rounded-[var(--radius-md)] border-2 border-[color:var(--color-accent)] bg-[color:var(--color-accent-tint)] p-5">
        ...
        {/* Phase 31  replacement only while collection is enabled. */}
        {collectionEnabled && (
          <>
            <input
              ref={fileRef}
  ...
  // Phase 31  while collection is dormant, never prompt for a document.
  // (The parent normally doesn't render the panel in this state at all;
  // this is the defensive in-component guard.)
  if (!collectionEnabled) {
    return (
      <div className="rounded-[var(--radius-md)] border-2 border-dashed border-[color:var(--color-hairline)] bg-[color:var(--color-surface)] p-5">
        <div className="flex items-center gap-2 text-[0.7rem] uppercase tracking-[0.22em] text-[color:var(--color-ink-soft)]">
          <ShieldOff className="size-3.5" aria-hidden="true" />
          ID verification not active
        </div>
        <p className="mt-2 text-sm text-[color:var(--color-ink-soft)]">
          Sebenza doesn&rsquo;t ask for ID documents right now. Your profile
